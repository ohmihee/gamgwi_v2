...
import {
  AddFriend_REQUEST,
  CancelSubscribe_Request,
} from "../../reducers/subscribe";

const ViewContent = (props) => {
  let { title, content, nickName, hit, id, likeIdx, date, weather } =
    props.viewData;
    ...
    <>
      <ViewContentWrap>
        ...
        <TitleWrap>
          {title}
          <WeatherPosition>
            {weather !== "cloud" && weather !== "snow" && weather == "sun" ? (
              <WbSunnyRoundedIcon />
            ) : weather == "cloud" &&
              weather !== "snow" &&
              weather !== "sun" ? (
        ...
        <DateWrap>
          <p>작성자 : {nickName}</p>
          {friend ? (
            <p onClick={cancelFriendBtn} className="addFriendDiv">
              칭구칭구
            </p>
          ) : (
            <p className="addFriendDiv" onClick={addFriendBtn}>
              친구추가
            </p>
          )}
          <p> {ymd} </p>
