...
let post_view = async (req, res) => {
  ...
  let result = {};
  try {
    let view_hit = await Board.findOne({
      where: { id: viewIdx },
      attributes: ["hit"],
    });
    await Board.update(
      { hit: view_hit.dataValues.hit + 1 },
      { where: { id: viewIdx } }
    );
    let view = await Board.findOne({
      where: { id: viewIdx },
      attributes: [
        "title",
        "content",
    ...
    let friend = await Subscribe.findOne({
      where: { userid: userid, writer_name: view.dataValues.nickName },
    });
    if (friend == null) {
      result = {
        result: "OK",
...
let modify_succece = async (req, res) => {
  ...
  try {
    await Board.update(
      { title: writeTitle, content: writeContent, weather: todayWeather },
      { where: { id } }
    );
    let modifiedRes = await Board.findOne({
      where: { id },
      attributes: ["title", "content", "hit", "date"],
    });
    result = {
      modifiedRes,
      result: "OK",
...
let add_friend = async (req, res) => {
  ...
  try {
    ...
    // userid 가 현재 로그인한 사람
    // nickname은 userid가 친구 맺기 한 사람
    const check = await Subscribe.findAll({
      where: {
    ...
    if (check.length !== 0) {
      throw res.json({ result: "Fail" });
    }
    let resultData = await Subscribe.create({
      writer_name: nickName,
      userid,
      status: true,
    });
...
let subscribe_post = async (req, res) => {
  ...
  try {
    ...
    let boardId = await Like.findAll({
      where: { userid: data },
      attributes: ["board_id"],
    });
    ...
    if (boardId.length == 0) {
      ...
      for (let i = 0; i < boardId.length; i++) {
        let pushData = await Board.findAll({
          where: { id: boardId[i].dataValues.board_id },
          attributes: ["id", "title", "hit", "nickNAme", "category"],
        });
...
let cancel_friend = async (req, res) => {
  ...
  try {
    ...
    await Subscribe.destroy({
      where: { userid: userid, writer_name: nickName },
    });
    data = {
      result: "OK",
      data: false,
